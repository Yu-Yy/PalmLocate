...
def main():
    ...
    train_dataset = THUPALM.THUPALM(root=config.DATASET.ROOT, is_train=True, transform=True)
    trainloader = torch.utils.data.DataLoader(train_dataset, batch_size=config.TRAIN.BATCH_SIZE_PER_GPU* len(gpus),collate_fn=my_collate_fn,
                                              shuffle=True, pin_memory=True, num_workers = config.NUM_WORKERS, drop_last=False) #
...
def train(config, train_loader, model, optimizer, logging, global_step):
    ...
    model.train()
    for step, (img, patch_image, heatmap, angle) in enumerate(train_loader):
        img = img.cuda()
        ...
        loss = 10 * loss_heatmap #+ 0.1 * loss_angle
        ...
        if (global_step + 1) % config.PRINT_FREQ == 0:
            logging.info('Epoch: [{0}][{1}/{2}]\t'
                         ...
                         'Loss_angle {loss_angle.val:.4f} ({loss_angle.avg:.4f})\t'.format(
                          config.TRAIN.EPOCHS, step, len(train_loader), loss=loss_collect, loss_heatmap=loss_heatmap_collect, loss_angle=loss_angle_collect))
