...
def main():
    ...
    if os.path.isfile(best_file):
        logging.info("=> loading best_file '{}'".format(best_file))
        ...
        logging.info("=> loaded checkpoint '{}' (epoch {})"
                .format(best_file, checkpoint['epoch']))
...
def valid(config, valid_loader, model, logging):
    ...
    for step, (img, patch_image, heatmap, angle) in enumerate(valid_loader):
        ...
        start = time.time()
        heatmap_pred= model(img, patch_image) #, anglemap_pred 
        end = time.time()
        if step > 0:
            time_collect.update(end-start)
        # print('forward time is {}s'.format(end-start))
        ...
        del img, patch_image, heatmap, angle, start, end
    logging.info(f'Average time per batch: {time_collect.avg}s')
